...
def _decode_sensor_record(
    ...
    if sensor_type == 'door':
        return _decode_door(header, record, rssi, snr)
    # if sensor_type == 'pet':
...
def _decode_door(
    ...
    if len(raw_payload) != SENSOR_RECORD_SIZE:
        raise ValueError('Door payload must be 8 bytes: header + failed attempts + battery.')

    succesfull_attempts = _read_single_scaled(raw_payload[1:2])
    failed_attempts = _read_single_scaled(raw_payload[2:3])
    battery_pct = _read_battery_pct(raw_payload[3])
