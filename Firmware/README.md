...
Deep-sleep driven: wakes 2.5 s before the expected beacon, listens for up to 5 s, stays awake to build and transmit the payload at its assigned slot, then sleeps until the next beacon. A second EXT0 wakeup on the potentiometer button (GPIO32) allows local UI interaction at any time.
